...
{
    char *err_msg = 0;
    const char *sql = "CREATE TABLE IF NOT EXISTS vendas_caixa3 ("
...
void registrar_compra(sqlite3 *db, ItemCompra itens[], int num_itens, float total)
{
    ...
    const char *sql = "INSERT INTO vendas_caixa3(nome_produto, peso, preco_total, tipo_compra) VALUES(?, ?, ?, ?)";
...
int main()
{
    ...
    // Cria a tabela vendas_caixa3 se não existir
